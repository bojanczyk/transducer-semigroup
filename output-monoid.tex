...
semigroup-to-set functor, whose set of components is $1 \oplus 1$ (cf.~\Cref{rem:coproduct-as-polynomial-functor}), it is not \emph{finite} polynomial (the set $1\oplus1$ is infinite). That said, we have by naturality (cf.~appendix) that:
...
\begin{proof}[Proof of \Cref{lem:compute-next-configuration}]
  ...
  naturality of the output mechanism (as detailed in the appendix).
