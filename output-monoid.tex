...
(cf.~\Cref{rem:coproduct-as-polynomial-functor}), as the register functor.
...
To fit \Cref{def:functorial-sst}, we have to check that this is a rational
...
Our second use of this finiteness preservation assumption is to build the
...
\emph{deconstruction} function of \Cref{lem:views} followed by reconstruction is
...
copyless natural function in $A$. (The injectivity proof in \Cref{lem:views}
actually provides such a reconstruction function.)

\tito{thus completing the proof of Theorem~\ref{thm:regular-functions}. The
  details are in the appendix. }

\begin{proof}[Proof of \Cref{lem:compute-next-configuration}]
  \tito{rename $w,a,v$ to $u,v,w$}
