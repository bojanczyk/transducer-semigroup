...
\[
    ...
    \end{tikzcd}
    \]
We begin by introducing some notation, which allows us to track which parts of an output come from which parts of an input. The new notation will be explained using the running example of the duplicating function, with the transducer semigroup of \Cref{ex:duplicator}.
...
\begin{proof}
    ...
    \begin{claim}\label{claim:merge-factorized-output}
        ...
        \]
    \end{claim}
    \tito{I checked the claim for the new definition of factorized output function, but I don't know if the details are worth spelling out (it's purely mechanical). See also \Cref{lem:merge-middle} where I put an actual proof}
