...
The key observation is that an element of a \kl{coproduct} is fully determined from its \kl{shape} and \kl{views}, as stated in the following lemma. 

\begin{proposition}
\label{prop:views} Let $A_1,\ldots,A_n$ be semigroups. The \intro{deconstruction} function of type
\begin{align*}
...
\begin{proof}
    ...
    Start with the \kl{shape}, and replace the entries from $1$ with the semigroup elements used in the \kl{views}.
\end{proof}
...
\AP Like any injective function, \kl{deconstruction} admits a partial left inverse: a function
...
We call \intro{reconstruction} the unique left inverse that sends any input not in the image of deconstruction to the right component $1$. Then from the argument proving \Cref{prop:views}, one can also derive the following (which makes sense thanks to \Cref{rem:coproduct-as-polynomial-functor}).
