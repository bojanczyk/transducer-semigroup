...
\begin{example}
 Consider the \enquote{opposite semigroup} functor from \Cref{ex:functors}. Define the output mechanism to be the identity. Using this transducer semigroup, we can recognize the string reversal function.
 % $f : \Sigma^* \to \Sigma^*$. 
...
As we explain in the appendix, there are too many recognizability reflecting functions -- in fact, uncountably many from $\{a\}^*$ to itself -- to allow a machine model, or some other effective syntax. A (non-effective) syntax is given in the following theorem, which is proved the same way as Theorem~\ref{thm:all-functions}.
