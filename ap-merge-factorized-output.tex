...
The property that we want on \kl{factorized outputs} reduces immediately to the following lemma on \kl{vectorial outputs}, by taking $C = 1$.
...
\end{lemma}
\begin{proof}
    By expanding the definition of the \kl{vectorial output functions}, and by using the associativity of three-fold multiplication, we see that the diagram in the lemma statement corresponds to the outer rectangle below:
    \[\begin{tikzcd}
        [column sep=4cm]
        \functor A\times \functor A \times \functor C
        ...
        A \oplus C
    \end{tikzcd}\]
    The lower rectangle commutes by \kl{naturality} of the \kl{output mechanism}. The middle trapeze commutes because \kl{merging} is a semigroup homomorphism. Concerning the upper trapeze, it can be decomposed as the bifunctor $\times$ applied to two diagrams that we can analyze independently. We start by looking at the second of those, which is simpler:
    \[\begin{tikzcd}
        [column sep=4cm]
        \functor C
    ...
    which is an elementary property of the \kl{coproduct} $\oplus$ in any category (indeed, the generic categorical definition of \kl{merging} uses a co-paring, which \enquote{cancels out} with the \kl{co-projection}).
    ...
    \[\begin{tikzcd}
        ...
        \functor(A \oplus A \oplus C)
            \ar[rru,"F(\text{merge})"']
    \end{tikzcd}\]
    The upper right trapeze commutes because $\functor$ applied to the \kl{co-projection} of $a$ into $A\oplus C$ is a homomorphism. The lower triangle commutes because $\functor(\text{\kl{merge}})$ is a homomorphism. Finally, the small upper left triangle can be shown to commute using the functoriality of $\functor$ followed by properties of the \kl{coproduct}.
\end{proof}
