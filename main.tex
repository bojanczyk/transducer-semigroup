...
\begin{proof}
    ...
    \[\begin{tikzcd}
        ...
        &
        \functor A \times \functor B \times \functor C
        \ar[dd,"\functor(\text{co-projection})^3"]
        \\
        ...
        \ar[r,"F(A \oplus (\mathrm{id}_B\,\text{or}\,\mathrm{id}_B) \oplus C)^3"]
        &
        ...
        A \oplus B \oplus C
    \end{tikzcd}\]
    The lower rectangle commutes by naturality of the output function. The middle rectangle commutes because of the homomorphism property of $F(A \oplus (\mathrm{id}_B\,\text{or}\,\mathrm{id}_B) \oplus C)$. Concerning the upper rectangle, it can be decomposed as the functor $(\cdots \times \cdots \times \cdots)$ applied to three diagrams that we can analyze independently. The first of those is
    \[\begin{tikzcd}
        [column sep=4cm]
        \functor A
        ...
        &
        \functor A
        \ar[dd,"\functor(\text{co-projection})"]
        \\
        \functor(A \oplus B \oplus B \oplus C)
        \ar[d,"\mathrm{id}"']
        \\
    ...
    \[\begin{tikzcd}
        ...
        \ar[dr,"\text{semigroup op}"]
        \\
        \functor(A \oplus B \oplus B \oplus C)
            \ar[rr,"h = F(A \oplus (\mathrm{id}_B\,\text{or}\,\mathrm{id}_B) \oplus C)"]
        &
        &
        \functor(A \oplus B \oplus C)
    \end{tikzcd}\]
    The large upper right triangle commutes because $\functor$ applied to the co-projection of $B$ into $A\oplus B\oplus C$ is a homomorphism. The trapeze commutes because $h$ is a homomorphism. Finally, the small upper left triangle can be shown to commute using the functoriality of $\functor$ followed by properties of the coproduct.
\end{proof}
