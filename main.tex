...
\begin{proof}
  ...
  \proofsubparagraph{From a register transducer semigroup to a bounded-copy SST.}
  ...
  \end{itemize}
  One can verify that $\tterase_\Gamma(\delta^*(w))(q) = (s,\; \text{something fully determined by}\ \mu_{q,s})$ where $s$ is the 1st component of $h(w)$ for any $w\in\Sigma^*$ and $q\in Q$. Since $s$ lives in the finite semigroup~$S$, the monoid $\tterase_\Gamma(\delta^*(\Sigma^*))$ is finite. By \Cref{prop:bounded-copy-finiteness}, our SST is bounded-copy.

  \proofsubparagraph{From a bounded-copy SST to a register transducer semigroup.}

  Let $\mathcal{T} = (Q, q_0, R, \delta, \vec{u}_I, F)$ be a bounded-copy SST. The core of the construction (which does not entirely finish the job on its own) is the following:
  \begin{claim}
