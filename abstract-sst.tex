...
\begin{definition}[\intro{Copyless natural function}] \label{def:copyless} A
  \kl{natural function} $f : \functor A \to \functorg A$ is called \emph{copyless}
  if it arises from some natural transformation between the \kl{polynomial
    functors} $\functor$ and~$\functorg$ with the following property:
  when instantiated to the semigroup\footnote{The choice of the semigroup
    $(\Nat,+)$ in the \Cref{def:copyless} is not particularly important. For
    ...
    \Cref{lem:functorial-sst-complete}.} $(\Nat,+)$, the corresponding function of type $\functor \Nat \to \functorg \Nat$ 
