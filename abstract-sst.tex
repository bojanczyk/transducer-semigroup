...
computation to be initialized in a more liberal way, that may depend on a
...
\begin{definition}\label{def:functorial-sst}
    ...
    \item An \emph{initial function} $\Sigma^* \to \functorr A$ which is
      \emph{recognizable}.
    \item A polynomial \emph{final functor} $\functork$, a designated
      \emph{final value} in $\functork A$, and a \emph{final function} of type
      $\functorr A \times \functork A \to A$, which is natural and copyless.
    \item An \emph{update oracle}, which is a rational function of type $\Sigma^* \to (\functoru A)^*$.
    \end{itemize}
\end{definition}
The semantics of the functorial \sst is a partial function of type 
$\Sigma^* \to A$ defined as follows. For every input string, we feed it to the
...
We then apply the final function to the last register valuation and to the
designated final value, yielding a result in $A$.
