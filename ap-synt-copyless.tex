...
Consider two \kl{monomial functors}, say $A^k$ and $A^\ell$, for some  $k,\ell \in \mathbb{N} = \set{0,1,\ldots}$.
...
We begin by looking at the \kl{components}.  Let $q_i \in Q$ be the \kl{component} of the register valuation $v_i$. The first observation is that $q_i$ depends only on $q_{i-1}$ and the $i$-th register update. Therefore, the sequence $q_1 \cdots q_n$ can be produced by a \kl{rational function}. The next observation is that, one we know the components $q_{i-1}$ and $q_{i}$, and the register update $u_i \in \functoru A$ that would be applied in the original transducer to from $v_{i-1}$ to $v_i$, then we can create a \kl{copyless} \kl{register update} that transforms $w_{i-1}$ into $w_i$. This is done by using the \kl{syntactic descriptions} of \kl{natural functions} that were described above. Once we have the register valuations $w_1,\ldots,w_n$, the output of the transducer can be easily obtained.
